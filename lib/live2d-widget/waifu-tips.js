...
function initWidget(waifuPath = "/waifu-tips.json", apiPath = "") {
	...
	function registerEventListener() {
		...
		$("#waifu-tool .fa-times").click(function() {
			...
			$("#waifu").animate({ bottom: -1000 }, 3000, function() {
				$("#waifu").hide(3000);
			});
		});
		var re = /x/;
	...
	var userAction = false,
		hitokotoTimer = null,
		messageTimer = null,
		messageArray = ["和你在一起感觉时间过得好快", "我可爱吗，我可爱吗，快说我可不可爱嘛", "好无聊～快来陪我玩吧！", "拿小拳拳锤你胸口！"];
	...
	function showHitokoto() {
		...
		if (Math.random() < 0.6 && messageArray.length > 0) showMessage(messageArray[Math.floor(Math.random() * messageArray.length)], 6000, 9);
				...
				var text = `你知道吗，这句话来自 <span style="color:#0099cc;">『${result.from}』</span>，是 <span style="color:#0099cc;">${result.creator}</span> 在 hitokoto.cn 投稿的。`;
